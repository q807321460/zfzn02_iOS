        ...
        set{
            ...
            UIView.animate(withDuration: 0.2, animations: { () -> Void in
                ...
                for btn in self.items {
                    ...
                    tempBtn.setTitleColor(UIColor.black, for: UIControlState())//未选中的为黑色
                    tempBtn.titleLabel?.font = UIFont.systemFont(ofSize: 14)
                }
                itemBtn.setTitleColor(self.lineColor, for: UIControlState())//和线条的颜色一致
...
//        for var index = 0; index < itemsTitles.count; index += 1 {
        ...
        btn.setTitleColor(lineColor, for: UIControlState())//保证颜色和线条是一致的
    ...
    fileprivate func setLineWithItensWidth(_ width:CGFloat) {
        line = UIView(frame: CGRect(x: 2, y: kScNavBarHeight - 3, width: width - 4, height: 3))
    ...
    fileprivate func showLaunchMenu(_ show:Bool) {
        if show {
            ...
            UIView.animate(withDuration: 0.5, animations: { () -> Void in
                ...
                self.arrowBtnImageView.transform = CGAffineTransform(rotationAngle: CGFloat(Double.pi))
