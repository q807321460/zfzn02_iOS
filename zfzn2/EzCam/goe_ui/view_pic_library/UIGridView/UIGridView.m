...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	CGFloat height = (float)[uiGridViewDelegate gridView:self heightForRowAt:(int)(indexPath.row)];
	CGFloat width = (float)[uiGridViewDelegate gridView:self widthForColumnAt:(int)(indexPath.row)];
    float space=(self.frame.size.width-width*numCols)/(numCols+1);
    CGFloat x = space;
...
- (IBAction) cellPressed:(id) sender
{
	...
	[uiGridViewDelegate gridView:self didSelectRowAt:cell.rowIndex AndColumnAt:cell.colIndex with_cell:cell];
}
