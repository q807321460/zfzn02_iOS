...
- (void)loadView
{
    ...
    _thumbsView							= [[UIScrollView alloc] initWithFrame:CGRectZero];//zxy------
   ...
   // _caption.textAlignment						= UITextAlignmentCenter;
    ...
    [leftButton setTintColor:UIColorFromRGB(m_strings.text_gray)];
...
- (void)reloadGallery
{
    _currentIndex = f_shared.nDefaultIndex;//_startingIndex;
...
- (void)setUseThumbnailView:(BOOL)useThumbnailView
{
    ...
    if( self.navigationController ) {
        if (_useThumbnailView) {
            ...
            UIBarButtonItem *btn = [[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"m_look_all", @"") style:UIBarButtonItemStylePlain target:self action:@selector(handleSeeAllTouch:)] autorelease];
            [btn setTintColor:UIColorFromRGB(m_strings.text_gray)];
...
{
    int barHeight = 20;
    if (MyShareData.screen_y_offset>0) {
        barHeight += 44;
    }
...
- (void)updateTitle
{
    //self sett
   // titleLabel.text=
    [titleLabel setText:[NSString stringWithFormat:@"%i of %i", _currentIndex+1, [_photoSource numberOfPhotosForPhotoGallery:self]]];
...
- (void)moveScrollerToCurrentIndexWithAnimation:(BOOL)animation
{
    //NSLog(@"moveScrollerToCurrentIndexWithAnimation...");
...
- (void)showThumbnailViewWithAnimation:(BOOL)animation
{
    ...
    [self arrangeThumbs];
    [self.navigationItem.rightBarButtonItem setTitle:NSLocalizedString(@"m_look_onebyone", @"")];
...
{
    _isThumbViewShowing = NO;
    f_shared.bThumbViewShowing=true;
    [self.navigationItem.rightBarButtonItem setTitle:NSLocalizedString(@"m_look_all", @"")];
