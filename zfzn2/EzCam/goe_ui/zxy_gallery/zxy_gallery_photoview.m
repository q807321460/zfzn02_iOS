...
@implementation zxy_gallery_photoview

@synthesize photoDelegate;
@synthesize imageView;
@synthesize activity = _activity;
@synthesize button = _button;
@synthesize b_fullloaded;
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (touch.tapCount == 2) {
        [self stopTapTimer];
        BOOL bdeal=false;
        if([photoDelegate respondsToSelector:@selector(didDoubleTapPhotoView:)])
            bdeal=[photoDelegate didDoubleTapPhotoView:self];
        if (bdeal==false) {
            return;
        }
