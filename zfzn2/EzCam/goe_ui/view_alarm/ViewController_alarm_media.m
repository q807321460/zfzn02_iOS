...
- (void)viewDidLoad {
    ...
    [button_video_pic setTitle:NSLocalizedString(@"m_alarm_pic", @"") forState:UIControlStateNormal];
    ...
    if(tableview_video==tableView)
    {
        ...
        if (cur_item.m_item_type==0) {
            ...
            if (m_share_item.cur_cam_list_item.m_fisheyetype<=0) {
                ...
                zxy_fish_playbackViewController_alarm* destview=nil;
                destview = [[zxy_fish_playbackViewController_alarm alloc]initWithNibName:@"zxy_fish_playbackViewController_alarm" bundle:nil];
                [self presentViewController:destview animated:YES completion:nil];
            }
        }
