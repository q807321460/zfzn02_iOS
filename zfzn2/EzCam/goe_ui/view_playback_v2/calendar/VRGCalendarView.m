...
-(void)showNextMonth {
    ...
    UIView *animationHolder = [[UIView alloc] initWithFrame:CGRectMake(0, kVRGCalendarViewTopBarHeight_vv, kVRGCalendarViewWidth_vv, targetSize-kVRGCalendarViewTopBarHeight_vv)];
    ...
    if (hasNextMonthDays) {
        animationView_B.frameY = animationView_A.frameY + animationView_A.frameHeight - (kVRGCalendarViewDayHeight_vv+3);
    } else {
    ...
    [UIView animateWithDuration:.35
                     animations:^{
                         ...
                         if (hasNextMonthDays) {
                             animationView_A.frameY = -animationView_A.frameHeight + kVRGCalendarViewDayHeight_vv+3;
                         } else {
...
-(void)showPreviousMonth {
    ...
    if (hasPreviousDays) {
        animationView_B.frameY = animationView_A.frameY - (animationView_B.frameHeight-kVRGCalendarViewDayHeight_vv) + 3;
    } else {
    ...
    [UIView animateWithDuration:.35
                     animations:^{
                         ...
                         if (hasPreviousDays) {
                             animationView_A.frameY = animationView_B.frameHeight-(kVRGCalendarViewDayHeight_vv+3);
...
-(float)calendarHeight {
    return kVRGCalendarViewTopBarHeight_vv + [self numRows]*(kVRGCalendarViewDayHeight_vv+2)+1;
}
...
{       
    ...
    if (touchPoint.y > kVRGCalendarViewTopBarHeight_vv) {
        float xLocation = touchPoint.x;
        float yLocation = touchPoint.y-kVRGCalendarViewTopBarHeight_vv;
        
        int column = floorf(xLocation/(kVRGCalendarViewDayWidth_vv+2));
        int row = floorf(yLocation/(kVRGCalendarViewDayHeight_vv+2));
...
-(int) get_vv_day_flag:(int)day with_arrays:(NSMutableArray*)flags_array
{
    int i=0;
    if (flags_array==nil||day<1 || day>flags_array.count) {
        return 0;
    }
    i=[[flags_array objectAtIndex:day-1]intValue];
    //NSLog(@"get_vv_day_flag===%d",i);
    return i;
}
-(NSString*)get_cur_month_str
{
    ...
    NSString* month_str=[nsdf2 stringFromDate:currentMonth];
    return month_str;
}
#pragma mark - Drawing
- (void)drawRect:(CGRect)rect
{
    ...
    NSMutableArray* m_days_flag_array=nil;
    if ([delegate respondsToSelector:@selector(calendarView:getDaysFlag:)])
        m_days_flag_array=[delegate calendarView:self getDaysFlag:self.currentMonth];
    ...
    CGRect rectangle = CGRectMake(0,0,self.frame.size.width,kVRGCalendarViewTopBarHeight_vv);
    CGContextAddRect(context, rectangle);
    CGContextSetFillColorWithColor(context, [UIColor whiteColor].CGColor);//顶部颜色
    ...
    attrs[NSFontAttributeName] = [UIFont fontWithName:@"HelveticaNeue" size:10];
    
    
    for (int i =0; i<[weekdays count]; i++) {
        NSString *weekdayValue = (NSString *)[weekdays objectAtIndex:i];
        UIFont *font = [UIFont fontWithName:@"HelveticaNeue" size:10];
        [weekdayValue drawInRect:CGRectMake(i*(kVRGCalendarViewDayWidth_vv+2), 40, kVRGCalendarViewDayWidth_vv+2, 20) withFont:font lineBreakMode:NSLineBreakByClipping alignment:NSTextAlignmentCenter];
        //[weekdayValue drawInRect:CGRectMake(i*(kVRGCalendarViewDayWidth_vv+2), 40, kVRGCalendarViewDayWidth_vv+2, 20) withAttributes:attrs];
    }
    ...
    //Grid background
    float gridHeight = numRows*(kVRGCalendarViewDayHeight_vv+2)+1;
    CGRect rectangleGrid = CGRectMake(0,kVRGCalendarViewTopBarHeight_vv,self.frame.size.width,gridHeight);
    ...
    CGContextBeginPath(context);
    CGContextMoveToPoint(context, 0, kVRGCalendarViewTopBarHeight_vv+1);
    CGContextAddLineToPoint(context, kVRGCalendarViewWidth_vv, kVRGCalendarViewTopBarHeight_vv+1);
    for (int i = 1; i<7; i++) {
        CGContextMoveToPoint(context, i*(kVRGCalendarViewDayWidth_vv+1)+i*1-1, kVRGCalendarViewTopBarHeight_vv);
        CGContextAddLineToPoint(context, i*(kVRGCalendarViewDayWidth_vv+1)+i*1-1, kVRGCalendarViewTopBarHeight_vv+gridHeight);
        
        if (i>numRows-1) continue;
        //rows
        CGContextMoveToPoint(context, 0, kVRGCalendarViewTopBarHeight_vv+i*(kVRGCalendarViewDayHeight_vv+1)+i*1+1);
        CGContextAddLineToPoint(context, kVRGCalendarViewWidth_vv, kVRGCalendarViewTopBarHeight_vv+i*(kVRGCalendarViewDayHeight_vv+1)+i*1+1);
    }
    ...
    CGContextBeginPath(context);
    CGContextMoveToPoint(context, 0, kVRGCalendarViewTopBarHeight_vv);
    CGContextAddLineToPoint(context, kVRGCalendarViewWidth_vv, kVRGCalendarViewTopBarHeight_vv);
    for (int i = 1; i<7; i++) {
        //columns
        CGContextMoveToPoint(context, i*(kVRGCalendarViewDayWidth_vv+1)+i*1, kVRGCalendarViewTopBarHeight_vv);
        CGContextAddLineToPoint(context, i*(kVRGCalendarViewDayWidth_vv+1)+i*1, kVRGCalendarViewTopBarHeight_vv+gridHeight);
        
        if (i>numRows-1) continue;
        //rows
        CGContextMoveToPoint(context, 0, kVRGCalendarViewTopBarHeight_vv+i*(kVRGCalendarViewDayHeight_vv+1)+i*1);
        CGContextAddLineToPoint(context, kVRGCalendarViewWidth_vv, kVRGCalendarViewTopBarHeight_vv+i*(kVRGCalendarViewDayHeight_vv+1)+i*1);
    }
    CGContextMoveToPoint(context, 0, gridHeight+kVRGCalendarViewTopBarHeight_vv);
    CGContextAddLineToPoint(context, kVRGCalendarViewWidth_vv, gridHeight+kVRGCalendarViewTopBarHeight_vv);
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        int targetRow = i/7;
        int targetX = targetColumn * (kVRGCalendarViewDayWidth_vv+2);
        int targetY = kVRGCalendarViewTopBarHeight_vv + targetRow * (kVRGCalendarViewDayHeight_vv+2);
        ...
        { //current month
            ...
            if (m_days_flag_array != nil && [self get_vv_day_flag:targetDate with_arrays:m_days_flag_array]==1) {
                CGRect rectangleGrid = CGRectMake(targetX+2,targetY+2,kVRGCalendarViewDayWidth_vv,kVRGCalendarViewDayHeight_vv);
                CGContextAddRect(context, rectangleGrid);
                CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:@"0x2ca7ea"].CGColor);
                CGContextFillPath(context);
        ...
        [date drawInRect:CGRectMake(targetX+2, targetY+10, kVRGCalendarViewDayWidth_vv, kVRGCalendarViewDayHeight_vv) withFont:[UIFont fontWithName:@"HelveticaNeue-Bold" size:17] lineBreakMode:NSLineBreakByClipping alignment:NSTextAlignmentCenter];
    }
    bDrawFlag=false;
...
-(UIImage *)drawCurrentState {
    float targetHeight = kVRGCalendarViewTopBarHeight_vv + [self numRows]*(kVRGCalendarViewDayHeight_vv+2)+1;
    
    UIGraphicsBeginImageContext(CGSizeMake(kVRGCalendarViewWidth_vv, targetHeight-kVRGCalendarViewTopBarHeight_vv));
    CGContextRef c = UIGraphicsGetCurrentContext();
    CGContextTranslateCTM(c, 0, -kVRGCalendarViewTopBarHeight_vv);    // <-- shift everything up by 40px when drawing.
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        bDrawFlag=false;
        kVRGCalendarViewWidth_vv=frame.size.width;
        kVRGCalendarViewTopBarHeight_vv=60;
        kVRGCalendarViewDayWidth_vv=kVRGCalendarViewWidth_vv/7-2;
        kVRGCalendarViewDayHeight_vv=(frame.size.height-60)/6-2;
        self.contentMode = UIViewContentModeTop;
        self.clipsToBounds=YES;
        
        isAnimating=NO;
        self.labelCurrentMonth = [[UILabel alloc] initWithFrame:CGRectMake(34, 15, kVRGCalendarViewWidth_vv-68, 40)];
        //[self.labelCurrentMonth setBackgroundColor:[UIColor orangeColor]];
        [self addSubview:labelCurrentMonth];
        labelCurrentMonth.backgroundColor=[UIColor whiteColor];
        labelCurrentMonth.font = [UIFont fontWithName:@"HelveticaNeue-Bold" size:14];
...
- (void)awakeFromNib {
    [super awakeFromNib];
    bDrawFlag=false;
    kVRGCalendarViewWidth_vv=CGRectGetWidth(self.bounds);
    kVRGCalendarViewTopBarHeight_vv=60;
    kVRGCalendarViewDayWidth_vv=kVRGCalendarViewWidth_vv/7-2;
    kVRGCalendarViewDayHeight_vv=(CGRectGetHeight(self.bounds)-60)/6-2;
    self.contentMode = UIViewContentModeTop;
    self.clipsToBounds=YES;
...
-(id)init:(float)width with_height:(float)height
{
    ...
    bDrawFlag=false;
    kVRGCalendarViewWidth_vv=width;
    kVRGCalendarViewTopBarHeight_vv=60;
    kVRGCalendarViewDayWidth_vv=kVRGCalendarViewWidth_vv/7-2;
    kVRGCalendarViewDayHeight_vv=(height-60)/6-2;
    //kVRGCalendarViewDayHeight_vv=kVRGCalendarViewDayWidth_vv;//(height-70)/6-3;
    self = [super initWithFrame:CGRectMake(0, 0, kVRGCalendarViewWidth_vv, 0)];
    if (self) {
        self.contentMode = UIViewContentModeTop;
        self.clipsToBounds=YES;
