...
- (void) moveAlert: (UIAlertView *) alertView
{
	...
	if (![self isLandscape]) alertView.center = CGPointMake(160.0f, 180.0f);
	else alertView.center = CGPointMake(240.0f, 90.0f);
...
+ (void) say: (id)formatstring,...
{
	...
	va_end(arglist);
	[ModalAlert ask:statement withCancel:NSLocalizedString(@"OK", nil) withButtons:nil];
	[statement release];
}
...
+ (BOOL) confirm: (id)formatstring,...
{
	...
	id statement = [[NSString alloc] initWithFormat:formatstring arguments:arglist];
	va_end(arglist);
	BOOL answer = [ModalAlert ask:statement withCancel:NSLocalizedString(@"Cancel", nil) withButtons:[NSArray arrayWithObject:NSLocalizedString(@"OK", nil)]];
	[statement release];
	return	answer;
}
