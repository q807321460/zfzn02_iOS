...
class SQLiteDB:NSObject {
	...
	@discardableResult func openDB(copyFile:Bool = true) -> Bool {
		if db != nil {
			closeDB()
		}
	...
	@discardableResult func execute(sql:String, parameters:[Any]? = nil)->Int {
	...
	@discardableResult func query(sql:String, parameters:[Any]? = nil)->[[String:Any]] {
		assert(db != nil, "Database has not been opened! Use the openDB() method before any DB queries.")
		var rows = [[String:Any]]()
		queue.sync {
			if let stmt = self.prepare(sql:sql, params:parameters) {
