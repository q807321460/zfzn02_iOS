 ...
 *         -1, 接口调用失败
 */
- (NSInteger)playCloud:(NSString*)accessTok devID:(NSString*)deviceID recordID:(int64_t)recordID timeOut:(NSInteger)timeOut;
/**
 *  播放云录像(可设置接口调用超时时长)
 *
 *  @param accessTok 管理员token/用户token
 ...
 *         -1, 接口调用失败
 */
- (NSInteger)playCloud:(NSString*)accessTok devID:(NSString*)deviceID recordID:(int64_t)recordID Type:(NSInteger)type timeOut:(NSInteger)timeOut;
/**
 *  停止云录像播放
 *
 *  @return 0, 接口调用成功
